...
def npu_recurrent_kda(
    ...
    *,
    ...
    if cu_seqlens is None:
        cu_values = tuple(i * dense_seq_len for i in range(batch + 1)) if not is_tnd else (0, total_tokens)
    elif isinstance(cu_seqlens, torch.Tensor):
        if cu_seqlens.dim() != 1 or cu_seqlens.dtype not in (torch.int32, torch.int64):
            raise RuntimeError("npu_recurrent_kda: cu_seqlens tensor must be 1D INT32 or INT64.")
    ...
    if len(cu_values) < 2 or cu_values[0] != 0 or cu_values[-1] != total_tokens:
        raise RuntimeError("npu_recurrent_kda: cu_seqlens must start at 0 and end at total_tokens.")
    if any(end < start or end - start > 8 for start, end in zip(cu_values, cu_values[1:])):
        raise RuntimeError("npu_recurrent_kda: cu_seqlens must be nondecreasing and each sequence length <= 8.")
