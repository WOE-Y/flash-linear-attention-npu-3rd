...
def recurrent_kda_reference(
    ...
    *,
    ...
    if use_qk_l2norm_in_kernel:
        q_flat = q_flat / torch.sqrt(torch.sum(q_flat * q_flat, dim=-1, keepdim=True) + 1e-6)
        k_flat = k_flat / torch.sqrt(torch.sum(k_flat * k_flat, dim=-1, keepdim=True) + 1e-6)
    q_flat = q_flat * scale
    ...
    final_state = state if state_v_first else state.transpose(-1, -2).contiguous()
    return _restore_layout(out_flat.to(q.dtype), v, layout), final_state.to(state_dtype)


recurrent_kda_golden = recurrent_kda_reference
