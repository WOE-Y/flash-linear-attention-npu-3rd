    ...
    aclOpExecutor *executor)
{
    ...
    const aclTensor *cuSeqlens = executor->ConvertToTensor(cuSeqlensOptional, DataType::DT_INT64);
    if (cuSeqlens == nullptr) {
        OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "failed to convert cuSeqlensOptional to tensor.");
    ...
    auto ret = ADD_TO_LAUNCHER_LIST_AICORE(
        RecurrentKda,
        OP_INPUT(query, key, value, gate, beta, initialStateRef, cuSeqlens, ssmStateIndicesOptional,
